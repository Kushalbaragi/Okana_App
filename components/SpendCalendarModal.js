...
function SpendCalendarModal({ open, onClose, transactions, recap }) {
  ...
  return (
    <Fragment>
      ...
      {/* A true bottom sheet in its own stacked Modal (not a card appended
          below the calendar) — dim/blur are zeroed out since the calendar's
          own backdrop is already dimming the screen behind both. */}
      <AnimatedModal
        open={!!selectedDate}
